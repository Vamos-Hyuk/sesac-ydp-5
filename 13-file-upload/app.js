...
const uploadDetail = multer({
  ...
  // 에러가 없음으로 "null"이라고 전달하여 콜백 함수를 호출!
...
// multer 설정(upload 변수)에 따랄 파일을 업로드 한 후, req.file 객체 생성
...
// multer 설정(upload 변수)에 따라 파일을 업로드 한 후, req,files 객체 생성
...
app.post(
  ...
  (req, res) => {
    console.log(req.files); // [ {파일1_정보}, {파일2_정보}, .. ] : 배열 형태로 각 파일 정보를 출력
    console.log(req.body);
    res.send('여러 개의 인풋에 각각 파일 업로드 완료!');
  }
);
