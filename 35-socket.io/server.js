...
const PORT = 8080;
// {socket.id: nick1, socket.id: nick2, ... }
const nickObjs = {};
...
io.on('connection', (socket) => {
  ...
  //     console.log(`${data.who} : ${data.msg}`);
  //     socket.emit('studyKr', { who: 'study', msg: '열공!!' });
  //   });
  ...
  // io.emit('notice', `${socket.id}님이 채팅방에 입장하셨습니다.`);
  ...
  socket.on('setNick', (nick) => {
    ...
    if (Object.values(nickObjs).indexOf(nick) > -1) {
      ...
      io.emit('notice', `${nick} 님이 채팅방에 입장했습니다.`); // 전체 공지
      socket.emit('entrySuccess', nick); // 해당 소켓 데이터 전송
      io.emit('updateNick', nickObjs); // 전체 사용자 닉네임 모음을 전달
      updateLists();
    }
  });
